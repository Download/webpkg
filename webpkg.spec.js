var log = require('ulog')('webpkg:spec');
...
describe('webpkg', function(){
	...
	it('inherits properties from the `webpack` configuration', function(){
		...
		expect(cfg.plugins.length).to.equal(4)
		expect(cfg.plugins[1]).to.equal('test')
	})
	...
	it('applies `pluginsPre` from the `webpack` configuration', function(){
		expect(cfg).to.have.a.property('plugins')
		expect(cfg.plugins).to.have.a.property('length')
		expect(cfg.plugins.length).to.equal(4)
		if (process.env.NODE_ENV) 
			expect(cfg.plugins[0]).to.equal('test-plugin')
		else
			expect(cfg.plugins[0]).to.equal('basePre')
	})


	it('applies `pluginsPost` from the `webpack` configuration', function(){
		expect(cfg).to.have.a.property('plugins')
		expect(cfg.plugins).to.have.a.property('length')
		expect(cfg.plugins.length).to.equal(4)
		if (process.env.NODE_ENV) { 
			expect(cfg.plugins[2]).to.be.an.instanceof(webpack.DefinePlugin)
			expect(cfg.plugins[3]).to.be.an.instanceof(webpack.optimize.DedupePlugin)
		}	else {
		  expect(cfg.plugins[2]).to.equal('basePost')
		}
	})
})
